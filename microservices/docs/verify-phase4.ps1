...
$rabbit
if ($rabbit -notmatch 'healthy') { throw 'RabbitMQ is not healthy' }
...
if ($giftResp -notmatch '200 OK') { throw 'Catalog seed or route failed' }
Write-Host 'PASS: Catalog seeded and reachable'

Write-Host '== STEP 3: Register/login/purchase =='
$registerBody = @{ username='phase4user'; email='phase4user@example.com'; phone='0500001111'; password='Pass123!' } | ConvertTo-Json
...
$loginBody = @{ username='phase4user'; password='Pass123!' } | ConvertTo-Json
...
$queues
if ($queues -notmatch 'product-catalog.gift-purchased' -or $queues -notmatch 'inventory.gift-purchased') {
  throw 'Expected queues not found'
}
